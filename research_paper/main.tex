...
On the popular \texttt{requests} library (1,433 nodes, 2,234 edges), Rabt answers the question ``Who calls \texttt{request}?'' using only 11 nodes (0.77\% of the graph) and 85 prompt tokens versus 31,661 tokens for full-repository context, achieving 99.70\% token reduction.
On a sample repository with mutation tracking, Rabt reduces context by 97.6\% (2,197 characters $\rightarrow$ 53 characters).
These results demonstrate that explicit dependency graphs enable dramatic cost reduction: at \$0.30 per million tokens (Gemini 2.5 Flash pricing), Rabt reduces query costs from \$0.0095 to \$0.000026---a 365$\times$ improvement that makes high-intelligence reasoning models (e.g., OpenAI~o1) economically viable for mass-scale code analysis.
\end{abstract}
...
For the query ``Where is \texttt{userData} modified?'', we compare full-repo context against Rabt's minimal subgraph using Gemini 2.5 Flash.
The full-repo baseline consumes 2,197 characters (660 prompt tokens), while Rabt's minimal subgraph uses only 53 characters (65 prompt tokens), achieving 97.6\% character reduction and 90.2\% token reduction.
Both methods return correct answers with high confidence, demonstrating that explicit dependency graphs eliminate irrelevant context without sacrificing accuracy.
...
The minimal subgraph for this query contains only 11 nodes (0.77\% of the graph) and correctly identifies that the public helper functions \texttt{get}, \texttt{post}, \texttt{put}, \texttt{delete}, \texttt{head}, \texttt{options}, and \texttt{patch} each call \texttt{request}.

We validate token usage using Google's Gemini 2.5 Flash model via the official API.
The full-repository baseline sends the first 120k characters of concatenated Python files as context, consuming 31,661 prompt tokens (375,216 characters, 11,259 lines).
Rabt's minimal subgraph requires only 85 prompt tokens (136 characters, 7 lines), achieving \textbf{99.70\% token reduction}.
Both methods return identical, correct answers, demonstrating that graph-based context extraction preserves accuracy while dramatically reducing cost.

\begin{table}[t]
\centering
\caption{Token usage comparison on \texttt{requests} library}
\label{tab:token-comparison}
\begin{tabular}{@{}lrr@{}}
\toprule
\textbf{Metric} & \textbf{Full-Repo RAG} & \textbf{Rabt} \\
\midrule
Characters & 375,216 & 136 \\
Lines of code & 11,259 & 7 \\
Prompt tokens & 31,661 & 85 \\
Graph nodes used & 1,433 (100\%) & 11 (0.77\%) \\
\textbf{Token reduction} & --- & \textbf{99.70\%} \\
\bottomrule
\end{tabular}
\end{table}
...
Using measured token counts from Gemini 2.5 Flash API, we analyze the economic impact of Rabt's token reduction.
Table~\ref{tab:cost-analysis} shows cost per query at current API pricing.

\begin{table}[t]
\centering
\caption{Cost analysis at production scale (Gemini 2.5 Flash: \$0.30/1M input tokens)}
\label{tab:cost-analysis}
\begin{tabular}{@{}lrrr@{}}
\toprule
\textbf{Method} & \textbf{Tokens/Query} & \textbf{Cost/Query} & \textbf{Cost/1K Queries} \\
\midrule
Full-Repo RAG & 31,661 & \$0.0095 & \$9.50 \\
Rabt & 85 & \$0.000026 & \$0.026 \\
\midrule
\textbf{Reduction} & \textbf{99.70\%} & \textbf{365$\times$} & \textbf{\$9.47 savings} \\
\bottomrule
\end{tabular}
\end{table}

For reasoning models with higher pricing (e.g., OpenAI o1 at \$15 per million input tokens), the savings are even more substantial:
full-repo queries cost \$0.475 each versus \$0.0013 for Rabt---a 365$\times$ improvement that makes continuous repository-level analysis economically feasible at scale.

These cost reductions are critical for production deployment scenarios such as:
(1) continuous integration systems that analyze every commit,
(2) IDE copilots serving thousands of developers, and
(3) automated code review systems processing pull requests at scale.

\input{fig_tokens_requests}
\input{fig_cost_requests}

\subsection{Comparison with an IDE copilot (Cursor)}

To compare Rabt with a production IDE copilot, we evaluate the same repository and query in Cursor.
We open the \texttt{requests} codebase, focus \texttt{src/requests/api.py}, and issue the query:
``In this repo, who calls the function \texttt{request} in src/requests/api.py? Please list only the function names that directly call \texttt{request()}.''

Cursor correctly identifies the same seven callers as Rabt and full-repo RAG.
However, from the Cursor usage dashboard, this single query consumes approximately 14,007 new input tokens, 12,857 cache-read tokens, and 27,010 total tokens.
Table~\ref{tab:cursor-comparison} summarizes the comparison.

\begin{table}[t]
\centering
\caption{Comparison with Cursor IDE on same query}
\label{tab:cursor-comparison}
\begin{tabular}{@{}lrrr@{}}
\toprule
\textbf{System} & \textbf{Prompt Tokens} & \textbf{Answer} & \textbf{Reduction vs Full} \\
\midrule
Full-Repo RAG & 31,661 & Correct & Baseline \\
Cursor IDE & $\sim$14,007 & Correct & 55.8\% \\
\textbf{Rabt} & \textbf{85} & Correct & \textbf{99.7\%} \\
\bottomrule
\end{tabular}
\end{table}

While Cursor's internal retrieval heuristics reduce context compared to full-repo RAG, Rabt's graph-based approach achieves an additional 164$\times$ reduction compared to Cursor (14,007 $\rightarrow$ 85 tokens), demonstrating the value of explicit dependency graphs over semantic similarity and file-level heuristics.
...
Table~\ref{tab:comparison} summarizes key differences between Rabt and related systems.
