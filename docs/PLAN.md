...
## Actual Status Report (Code on Disk)

**You are not at Day 1.** Based on the code currently on your machine, the core engine (Weeks 1–8) is already implemented. This section reflects *actual* status; the week-by-week checklist below remains the original plan.

### ✅ What is ALREADY DONE (The "Banked" Wins)

| Phase   | Plan item           | Status | Evidence in your code |
|--------|---------------------|--------|------------------------|
| Week 1 | Static parser       | **DONE** | `parser/ast_parser.py` works (AST → nodes + edges). |
| Week 2 | Graph storage       | **DONE** | `graph/storage.py` saves/loads to JSON (e.g. `.graph.json`). |
| Week 3 | Runtime calls       | **DONE** | `.runtime_events.jsonl` logs calls; `@track_runtime` in sample repo. |
| Week 4 | Mutation tracking   | **DONE** | `track_mutation` and MODIFIED_BY edges; "Where is userData modified?" works. |
| Week 5 | Impact scoring      | **DONE** | `agent/subgraph.py`: subgraph text adds "(executed N times)" when weight > 1. |
| Week 6 | Query agent         | **DONE** | `agent/intent.py` maps queries to intents; traversal logic in place. |
| Week 7 | Minimal subgraph    | **DONE** | `agent/traversal.py` slices the graph; minimal subgraph extraction works. |
| Week 8 | LLM integration    | **DONE** | `llm/__init__.py` connects to Gemini; answers + confidence from subgraph. |

### 🚀 What is LEFT TO DO (Your Real Roadmap)

You are now entering **Week 9: The Polish Phase.** The core engine works; next is making it look strong and proving it robustly.

| Priority | Feature              | Why | Task |
|----------|----------------------|-----|------|
| **1**    | **Visualization**     | "Wow" factor; screenshot for the report. | Create `visualize_graph.py` that reads `.graph.json` and draws it (e.g. matplotlib or pyvis). Goal: red node = hot path (high execution count), blue = cold path. |
| **2**    | **Change detection** | You claimed the graph is "living." | Function that checks `git diff`; if e.g. `user_service.py` changed, re-run the parser only on that file and update the graph. |
| **3**    | **Baseline comparison** | Proof you are better than Cursor. | Pick a hard question (e.g. "Where is userData modified?"). Ask ChatGPT/Gemini with context = full file; record answer. Ask Rabt with context = subgraph; record answer. Show Rabt is cheaper (fewer tokens) and more accurate. |

### Mentor's advice on this document

**Do not change the plan.** Keep the week-by-week checklist and phase overview exactly as they are.

When you submit your final report, include this plan and say:

> *"We designed a 10-week implementation plan. Due to effective architectural choices (NetworkX + AST), we successfully accelerated the development and completed the core engine (Weeks 1–8) in the first phase."*

That positions you as highly competent: the plan was ambitious, and you delivered the core ahead of schedule.

### What is still optional (from the plan checklist)

Compared to the week-by-week checklist below, these items are **not yet implemented**. They are optional polish; the MVP is complete without them.

| From plan | Item | Status / note |
|-----------|------|----------------|
| **Week 5** | "Top N nodes by impact" (query or CLI) | Not implemented. You have edge weights and "(executed N times)" in subgraph text; no dedicated "top N by impact" query. |
| **Week 5** | Impact score formula (0.5×frequency + 0.5×downstream) | Not implemented. We use raw execution count as weight; no normalized 0–1 score or downstream count. |
| **Week 8** | Explainability: "why was this node included?" | Not implemented. Optional. |
| **Week 8** | Formal "5 ground-truth queries" test + correctness rate | Not automated. Baseline comparison runs one query; no 5-query correctness report. |
| **Week 9** | Re-parse **only** changed files (incremental) | Not implemented. We re-parse the **whole** repo when changes are detected; plan asked for incremental re-parse of single files. |
| **Week 9** | Git hook to trigger graph update | Not implemented. You have CLI `update_graph_if_changed.py`; no sample git hook. |
| **Week 9** | Validate: "change 1 file → graph updates in < 30 s" | Not validated. Could be added as a test or doc. |
| **Week 10** | Interactive viewer (click to expand) | Not implemented. We have static image (matplotlib); no Plotly/interactive viewer. |
| **Week 11+** | Unit tests for queries (`test_query_modification`, `test_subgraph_minimality`) | Not added. Existing tests cover parser/types; no dedicated query/subgraph tests. |
| **Week 11+** | Single "full evaluation" script (correctness + minimality + hallucination) | Not implemented. We have baseline_comparison (one query) and evaluation.protocol; no combined script. |

**Summary:** Core (Weeks 1–8) and Week 9 polish (visualization, change detection, baseline) are done. The table above is **optional** if you want to go further (incremental re-parse, git hook, explainability, query unit tests, full evaluation script).
...
- [x] Use `git diff` to detect changed files
- [x] Re-parse only changed files
- [x] Update graph: add/remove nodes and edges
- [ ] Recompute impact scores for affected subgraph
- [ ] Optional: git hook or CLI command to trigger update
- [x] Validate: change 1 file → graph updates in < 30 seconds
